classdef process_max_accy < handle
    properties
        to_run
    end

    methods
        function obj = process_max_accy(set)
            obj.to_run = process_max_accy.assess_to_run(set);
        end
    end

    methods (Static)
        function to_run = assess_to_run(Set)
            if Set.get_drop(1).signals('accy').data == false;
                to_run = false;
            else
                to_run = true;
            end
