classdef process_max_loady < handle
    properties
        to_run
    end

    methods
        function obj = process_max_loady(set)
            obj.to_run = process_max_loady.assess_to_run(set);
        end
    end

    methods (Static)
        function to_run = assess_to_run(Set)
            if Set.get_drop(1).signals('loady').data == false; 
                to_run = false;
            else
                to_run = true;
            end
